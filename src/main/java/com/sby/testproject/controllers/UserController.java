...
import com.sby.testproject.services.UserService;
...
public class UserController {
    ...
    public String getLogin(@SessionAttribute(value = "signedUser", required = false) UserEntity user) {
        if (user == null) {
            ...
            return hasMarathon ? "redirect:/user/marathon" : "redirect:/user/search"; // 진행 중인 마라톤이 있을 시 마라톤 페이지, 아닐 경우 검색 페이지로 이동
        }
//        return (user == null) ? "user/login" : "redirect:/";
    }
    ...
    public String postLogin(HttpSession session, UserEntity user) {
        ...
        if (result == CommonResult.SUCCESS) {
            ...
            response.put("redirect", hasMarathon ? "/user/marathon" : "/user/search");
        }
